...
# -*- coding: utf-8 -*-
import unittest
from incolumepy.scraping.google import GoogleSearch, os, BeautifulSoup, NavigableString
...
class HandleHTMLTest(unittest.TestCase):
    ...
    def test_filein_exists(self):
        ...
        self.assertEqual(os.path.realpath(self.path), os.path.realpath(self.google_search.filein))
    ...
    def test_content(self):
        ...
        self.assertIn('Google treinamento Incólume', self.google_search.content)
    ...
    def test_soup(self):
        ...
        self.assertEqual('Google treinamento Incólume', self.google_search.soup.index_title_tagname.text)
    ...
    def test_html_attrs(self):
        ...
        expected = {'itemscope': '', 'itemtype': 'http://schema.org/WebPage', 'lang': 'pt-BR'}
    ...
    def test_name_form_input(self):
        ...
        expecteds = ['input-hl', 'input-source', 'input-q', 'input-btnG', 'input-btnI', 'input-iflsig', 'input-gbv']
        for expected in expecteds:
            self.assertIn(expected, self.google_search.form_inputs_name().keys())

    def test_title_form_input(self):
        """Title do campo form.input"""
        expected = 'Pesquisa Google'
        self.assertEqual(expected, self.google_search.form_principal_input_title())
